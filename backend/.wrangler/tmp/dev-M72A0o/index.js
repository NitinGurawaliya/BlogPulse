...
// .wrangler/tmp/bundle-SIktui/checked-fetch.js
...
// .wrangler/tmp/bundle-SIktui/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-SIktui/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
userRouter2.get("/user/:id", authMiddleware, getProfile);
...
async function createBlog(c) {
  ...
  const body = await c.req.json();
  const { success } = import_medium_common3.createBlogSchema.safeParse(body);
  if (!success) {
...
async function editBlog(c) {
  ...
  const body = await c.req.json();
  const { success } = import_medium_common3.updateBlogSchema.safeParse(body);
  if (!success) {
    return c.json("Invalid inputs");
  }
  try {
